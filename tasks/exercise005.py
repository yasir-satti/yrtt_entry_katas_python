...
# the wave can sometimes pass through it. Usually only one wave crest will be present at any given time in an 
# arena, although simultaneous, counter-rotating waves have been produced. (Source Wikipedia)
...
def wave(people):
    ...
    if length != -1:
        for i in range(length):
            if people[i] != ' ':
                mex_word.append(people[:i:] + people[i].upper() + people[i + 1:length:])
    return mex_word
